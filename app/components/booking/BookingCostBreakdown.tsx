...
import type { PromotionPricingSegment } from "~/types/promotion-pricing";
import {
...
interface BookingCostBreakdownProps {
  ...
  readonly promotionPricingSegments?: PromotionPricingSegment[] | null;
...
export function BookingCostBreakdown({
  ...
  const hasSplitPricing = !!promotionPricingSegments && promotionPricingSegments.length > 0;

  const splitPricingRows = hasSplitPricing ? (
    promotionPricingSegments.map((segment, index) => {
      const segmentUnitLabel =
        segment.units === 1
          ? BOOKING_TYPE_LABELS[bookingType].singular
          : BOOKING_TYPE_LABELS[bookingType].plural;
      const segmentRateText = `${formatCurrency(segment.unitPrice)} × ${segment.units} ${segmentUnitLabel}`;

      return (
        <div
          key={`${segment.kind}-${segment.unitPrice}-${segment.compareAtUnitPrice ?? "none"}-${index}`}
          className="flex justify-between gap-2 mb-1.5"
          aria-label={segment.kind === "PROMO" ? "Promotional base price" : "Standard base price"}
        >
          <dt className="text-gray-600 min-w-0">
            {segment.kind === "PROMO" ? (
              <span className="tabular-nums">
                {segment.compareAtUnitPrice != null ? (
                  <span className="text-gray-400 line-through mr-1.5" aria-hidden="true">
                    {formatCurrency(segment.compareAtUnitPrice)}
                  </span>
                ) : null}
                <span className="text-primary">{segmentRateText}</span>
              </span>
            ) : (
              segmentRateText
            )}
          ...
          <dd className="font-medium shrink-0 text-right">
            <span aria-label="Payable base total">{formatCurrency(segment.total)}</span>
          </dd>
        </div>
      );
    })
