...
- Call the API from loaders and actions through one server-only client.
...
```text
GET /search?... -> RR loader -> API GET /api/cars/search?... -> HTML + route data
```
...
GET /bookings
  ...
  -> forwards Cookie to API GET /api/bookings
  -> the API's SessionGuard validates Better Auth session
...
The API's Better Auth currently uses:
...
2. The action calls the corresponding Better Auth endpoint on the API with the requested `role` in both OTP request and verification payloads.
3. The action copies all API `Set-Cookie` headers to its response.
4. The browser stores the host-only cookie for the web origin.
5. Later RR loaders receive that cookie and forward it server-to-server to the API.
...
Forward the trusted web `Origin` and a canonical web `Referer` for Better Auth mutations, and add production/preview web origins to the API `TRUSTED_ORIGINS`. The API uses the referer pathname to authorize requested entry roles:

- `/auth...` -> `user`;
- `/fleet-owner...` -> `fleetOwner`;
- `/admin...` -> `admin` or `staff`.

A server-to-server auth call that omits `Origin`, omits `role`, or forwards the wrong referer path will fail or be interpreted as a customer login. Build the referer from the validated application origin and known route; never accept an arbitrary caller-supplied URL. Do not weaken the API origin checks to `*`.

The Better Auth catch-all writes its own responses and does not pass through the API's global exception filter. Parse Better Auth error bodies and rate-limit headers separately from normal API Problem Details.
...
- send that value to the API as `X-Payment-Status-Token` for guest payment status, confirmation, and expiration requests;
...
app/
  lib/
    api/
      ...
      problem-details.ts     # normalized API error shape
...
app/
  ...
  lib/
    api/
      ...
      cars.server.ts         # capability-grouped API calls
      bookings.server.ts
      payments.server.ts
      reviews.server.ts
      referrals.server.ts
...
```text
route loader/action -> capability API module -> shared API client -> the API
...
but it does not recreate API aggregates, repositories, or domain services in
...
- Web modules never infer business permissions or state-transition rules that
  should be returned or enforced by the API.
...
No Swagger/OpenAPI setup was found in the API during this review. Mobile currently maintains manual TypeScript response types and casts parsed payloads.

Preferred direction:

1. Make the API's existing Zod DTOs the contract source of truth.
2. Either extract transport schemas/error codes/endpoint definitions into a framework-neutral `@hyre/api-contracts` package consumed by the API, web, and mobile, or generate OpenAPI from those schemas.
...
- mirror the API transport DTOs, not Prisma models;
...
Do not import API source files through cross-repository relative paths. That couples deployment and can accidentally bundle backend code.
...
- pass only validated values to the API.
...
Successful route actions automatically revalidate active React Router loaders. This keeps rendered route data synchronized; it does not invalidate the API caches, Cloudflare caches, or unrelated client caches. Those require explicit policy.
...
- Authorize in the API on every request; RR role checks are UX guards only.
...
```text
Browser -> Cloudflare Worker -> the API -> downstream service
```
...
- the API upstream duration and status;
- timeout and network-failure categories;
- Cloudflare colo and the API deployment version where available.
...
- `/openapi.json` only when it publishes or safely proxies an API-generated contract, not a separately maintained web specification;
- API documentation pages;
- content negotiation for website-owned representations.

Move or redirect webhooks, payment callbacks that perform business work, test activation routes, file proxying, and AI/business API routes to the API.

## Airport-completion link ownership

The current uncommitted implementations expose the same chauffeur page URL in both web and the API. Choose one owner before release.

Recommended:

- `hyre-web` owns `GET /chauffeur/airport-trips/:id/complete` and its confirmation UI;
- the API owns JSON read/complete operations and all authorization/token validation;
- notification links are generated from a dedicated canonical web origin, not API `AUTH_BASE_URL`;
- the API does not serve a competing HTML page at the public web URL.

Keep the existing API HTML endpoint only as a temporary fallback if required, with an explicit redirect/deprecation plan.
